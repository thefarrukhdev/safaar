...
const path = '/home/farrukh/Projects/Frontend/safaar/apps/web-user/components/ui/FilterSidebar.tsx';
...
// Headers
content = content.replace(
  /border-slate-100 pb-3\.5 dark:border-slate-800/g,
  'border-slate-900/[0.08] pb-3.5 dark:border-white/[0.10]'
);
content = content.replace(
  /border-slate-100 px-5 py-4 dark:border-slate-800/g,
  'border-slate-900/[0.08] px-5 py-4 dark:border-white/[0.10]'
);

// Title styling
content = content.replace(
  /text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white/g,
  'text-base font-semibold text-slate-900 dark:text-white'
);

// Reset button
content = content.replace(
  /text-xs font-extrabold text-blue-600 hover:text-blue-700 hover:underline dark:text-blue-400 cursor-pointer transition-colors/g,
...
content = content.replace(
  /text-xs font-bold text-blue-600 dark:text-blue-400/g,
  'text-sm font-medium text-blue-600 hover:text-blue-700 hover:underline dark:text-blue-400 cursor-pointer transition-colors duration-200 ease-[cubic-bezier(0.2,0,0,1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 rounded-full px-2 py-1'
);

// Action Button
content = content.replace(
  /w-full rounded-xl bg-blue-600 py-3\.5 px-4 text-sm font-extrabold text-white shadow-lg shadow-blue-600\/25 transition-all duration-200 hover:bg-blue-700 active:scale-\[0\.99\] cursor-pointer/g,
...
content = content.replace(
  /w-full rounded-xl bg-blue-600 py-3\.5 px-4 text-sm font-extrabold text-white shadow-lg shadow-blue-600\/25 transition-all hover:bg-blue-700 cursor-pointer/g,
  'w-full rounded-full bg-blue-600 h-12 px-6 flex items-center justify-center text-base font-medium text-white shadow-emboss-alpha transition-[transform,background-color] duration-200 ease-[cubic-bezier(0.2,0,0,1)] hover:bg-blue-700 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 motion-reduce:transition-none cursor-pointer'
);

// Mobile Drawer overlay
content = content.replace(
  /fixed inset-0 bg-slate-950\/60 backdrop-blur-xs transition-opacity/g,
  'fixed inset-0 bg-slate-900/50 dark:bg-black/60 transition-opacity'
);

// Drawer content sheet
content = content.replace(
  /relative z-10 flex max-h-\[85vh\] w-full flex-col rounded-t-3xl border-t border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 animate-in slide-in-from-bottom duration-300/g,
...
// Drag Handle
content = content.replace(
  /bg-slate-300 dark:bg-slate-700/g,
  'bg-slate-900/[0.12] dark:bg-white/[0.16]'
);

// X close button
content = content.replace(
  /flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:border-slate-800/g,
...
// Mobile footer
content = content.replace(
  /border-t border-slate-100 bg-slate-50 px-5 py-4 dark:border-slate-800 dark:bg-slate-950\/40/g,
