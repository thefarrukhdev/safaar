...
# 4. Multi-language Dynamic Content (AI Translation) - Backend Task

## Context
Foydalanuvchilar platformaga (web-user) rus yoki ingliz tilida kirganda, ba'zi mehmonxonalarning izohlari (description) baribir o'zbek tilida (fallback) ko'rsatilmoqda. Chunki mehmonxona egalari (partnerlar) faqat o'zbek tilida ma'lumot qoldirgan.
Buni professional tarzda hal qilish uchun backend'da avtomatik tarjima yoki AI yordamchisiga asoslangan yechimlar kerak.

## Nima qilish kerak?

Safaar platformasining arxitekturasiga mos ravishda 2 ta eng yaxshi yechimdan birini (yoki ikkalasini) amalga oshirish kerak:

### Variant A: Partner Panel API'da (On-Write Translation)
**Endpoint:** `POST /v1/hotels` yoki `PUT /v1/hotels/:id`
- Mehmonxona egasi ma'lumotlarni jo'natganda, agar `short_description` yoki `full_description` maydonlarida (yoki oddiy `description` da) faqat bitta til (masalan `uz`) kiritilgan bo'lib, `ru` va `en` tillari bo'sh bo'lsa:
- Backend fonda (background job/queue orqali) ushbu matnni Google Cloud Translation API, DeepL yoki Gemini API orqali rus va ingliz tillariga tarjima qiladi.
- Va to'liq qilib ma'lumotlar bazasiga (Database) saqlab qo'yadi.
- Natijada Frontend buni o'qiganda 3 tilda tayyor ma'lumot keladi, fallback bo'lmaydi.

### Variant B: AI Tarjimon Yordamchisi (AI Assistant Endpoint)
Partnerlarga ma'lumot kiritish jarayonida ko'maklashish uchun alohida endpoint yaratish:
**Endpoint:** `POST /v1/ai/translate`
- **Request Body:**
  ```json
  {
    "text": "QA/testing uchun maxsus test yotoqxona.",
    "source": "uz",
    "targets": ["ru", "en"]
  }
  ```
- **Response:**
  ```json
  {
    "ru": "Специальный тестовый хостел для QA/тестирования.",
    "en": "Special test hostel for QA/testing."
  }
  ```
- Ushbu usulda Partner matnni to'ldirayotganda bitta tugma bosadi (Generate). Frontend ushbu API'ni chaqiradi, olingan javoblarni `ru` va `en` maydonlariga joylashtiradi. Keyin Partner ularni o'qib tasdiqlab (Edit qilib), odatiy `POST /v1/hotels` orqali saqlaydi.

## Frontend integratsiyasi
Variant A tanlansa — Frontend (web-user) da hech narsa o'zgartirish shart emas, chunki turlar (LocalizedText) tayyor va ishlamoqda.
Variant B tanlansa — `web-partner` da shakl (Form) yoniga AI tugmasi qo'shiladi va u ushbu tarjima API'sini ishlatadi.
