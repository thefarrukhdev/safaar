...
test('debug: current --text-muted value + contrast detail on /users', async ({ page }) => {
  ...
  const varValue = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--text-muted'));
  console.log('CURRENT --text-muted VALUE=' + varValue);
  ...
  const detail = await page.evaluate(async () => {
    ...
    return c ? c.nodes.slice(0,3).map((n:any)=>({html:n.html.slice(0,150), summary:n.failureSummary})) : null;
  });
